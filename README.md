...
![前台操盘大屏](docs/images/v751_live_dashboard.png)

### 2. 后台机构级量化控制面（核心策略、数据健康与物理风控全景监控）
![后台机构级控制面](docs/images/v751_admin_overview.png)
...
```
     ...
     ▼                       ▼                             ▼                             ▼                       ▼
┌──────────────────┐  ┌──────────────────┐        ┌──────────────────┐          ┌──────────────────┐  ┌──────────────────┐
│ 1. 标的池全要素   │  │ 2. 提示词与插槽  │        │ 3. 物理风控门禁   │          │ 4. 投委会席位矩阵 │  │ 5. 策略大一统快照 │
│    100% 自定义   │  │    100% 自定义   │        │    100% 自定义   │          │    100% 自定义   │  │ 0.5s 秒级回滚    │
├──────────────────┤  ├──────────────────┤        ├──────────────────┤          ├──────────────────┤  ├──────────────────┤
│• 任意OKX合约增删 │  │• System/User 重写│        │• 独立Python插件化│          │• 参谋席位自由增删│  │• 4大策略单元打包 │
│• 面值/精度/最小量│  │• 积木式消息管线  │        │• 顺势/置信/ADX门禁│          │• 席位模型/温度解耦│ │• 唯一 SHA-256 溯源│
│• 基础仓位/风险额 │  │• 语义数据变量插槽│        │• 真实 2.0R 盈亏比│          │• 辩论/仲裁流程可调│ │• 0.5s 原子级热回滚│
│• 杠杆上限/ATR倍数│  │• JSON 方案导入导出│       │• 在线沙箱回归测试│          │• 思考链超时自由配│  │• 台账 100% 溯源绑定│
└──────────────────┘  └──────────────────┘        └──────────────────┘          └──────────────────┘  └──────────────────┘
```
...
![提示词策略工作室](docs/images/v751_prompt_studio.png)
...
![自进化认知实验室](docs/images/v751_self_evolution.png)
...
![物理拦截插件管线](docs/images/v751_interceptors_failclosed.png)
...
![对冲基金多模型决策委员会](docs/images/v751_council_board.png)
...
![策略大一统版本快照](docs/images/v751_policy_snapshot.png)
