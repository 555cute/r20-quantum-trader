...
- 标的池按内部合约 ID 纠正旧版误写的币种代码和自动名称（例如 DUSK 不再显示 BTC），保留自定义名称与已有风险参数；交易所显示代码仅在 API 响应中生成，不写入共享标的池。开仓计算优先使用当前合约实时返回的价格和数量规则。
- Binance 保护为两条配对条件单，**不是原子 OCO**。GTC 限价单受理但未成交时记录为 `awaiting_fill`，不会在空仓时提前发送 `closePosition` 止损/止盈。检测到部分成交后先撤余量并确认订单终态、核对实际仓位，再依次确认止损和止盈。
- Binance 零成交开仓限价单保留 AI 入场价，等待上限为 15 分钟；维护线程与交易周期清理共用同一阈值，超过后请求撤单，下一次查询确认终态后才解除阻断。提交动作会注明等待上限；维护任务将交易所确认的零成交终态、订单 ID 和是否请求过撤单写入后台 `order_maintenance` 成功任务详情，普通等待轮询不刷任务记录。延长等待不保证成交，不改变止损止盈或仓位预算；部分成交仍立即撤余量并进入保护流程。OKX 超时策略不变。
- Gateway 使用独立单任务维护线程，每约 5 秒检查一次待处理订单，并与交易周期共用互斥锁。成交到保护确认之间仍有轮询和网络延迟，不能视作原子保护；存在待处理或未知结果时，暂停新开仓和付费 AI 分析。
- 待处理状态保存在账户目录的 `pending_entries.json`。重启后按原客户端订单 ID 查询，不盲目重发；止盈失败保留已确认止损，止损明确失败时只补偿平掉本次成交数量，不动原底仓。无法确认的记录保持阻断，须核对交易所订单后处理，不能靠删除状态文件强行恢复交易。
