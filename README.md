...
- Binance 零成交开仓限价单保留 AI 入场价，等待上限为 15 分钟；维护线程与交易周期清理共用同一阈值，超过后请求撤单，下一次查询确认终态后才解除阻断。提交动作会注明等待上限，维护日志会记录交易所确认的零成交终态、订单 ID 和是否请求过撤单。延长等待不保证成交，不改变止损止盈或仓位预算；部分成交仍立即撤余量并进入保护流程。OKX 超时策略不变。
