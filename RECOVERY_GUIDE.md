...
- `.env`：原生部署的非公开配置与启动设置，位于项目根目录。
- `data/config/.env`：Docker 后台可写配置（`R20_ENV_FILE=/app/data/config/.env`）。宿主机 `.env` 只作启动注入，持久化文件覆盖它。打包的 data 备份会排除该文件，须单独安全保管与恢复。从旧镜像升级前，若容器内 `/app/.env` 已有后台修改，先导出到此路径。
...
- `data/exchanges/<exchange>/<mode>/<credential-fingerprint>/r20_quant.db`：按交易所、环境及凭证隔离的交易台账数据库。
- 策略配置：提示词库、投委会、拦截器配置及 `policy_archives/`。
- 账户运行状态：同一隔离目录下的台账、仓位追踪、熔断/冷却、决策记录、初始本金与权益快照。切换交易所或凭证不复用另一账户的文件。
...
旧版 `data/` 根目录下的账户文件不会自动迁移。不要直接把旧 OKX“张数”仓位追踪文件放进统一币数量的新目录，也不要把旧账户流水分配给新的 Binance 账户。先核对账户归属与单位，再决定人工迁移范围。SQLite 热备份在 `backups/sqlite/` 下保留账户子目录，不能将不同账户同名数据库摊平覆盖。
