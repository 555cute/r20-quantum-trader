...
import { isProtectionConfirmed } from '../../utils/protection'
...
const currentAtr = computed(() => {
  const it = factorItem.value
  if (!it) return 0
  const direct = Number(it.atr_1h)
...
const riskRewardMetrics = computed(() => {
  ...
  if (hasRealPosition && activePosition.value) {
    ...
    const rawLever = Number(activePosition.value.lever ?? 0)
    if (rawLever > 0) activeLeverage = rawLever
  }

  const estProfitUsd = activeMargin > 0 && activeLeverage > 0 ? activeMargin * activeLeverage * (rewardPct / 100) : null
  const estRiskUsd = activeMargin > 0 && activeLeverage > 0 ? activeMargin * activeLeverage * (riskPct / 100) : null
    ...
    getBars: async ({ callback }) => {
      try {
        const res = await fetch(
          `/api/v1/market/${currentInstId.value}/candles?bar=${currentPeriod.value}&limit=99&_t=${Date.now()}`,
          { cache: 'no-store' }
        )
...
function updatePriceLines() {
  ...
  const lineKind = simMode.value
    ? 'sim'
    : isProtectionConfirmed(activePosition.value)
      ? 'confirmed'
      : (activePosition.value || activeOrder.value ? 'requested' : 'none')
  const muted = lineKind === 'requested'
  const slColor = muted ? '#788370' : '#F43F5E'
  const tpColor = muted ? '#788370' : '#10B981'
  const dash = muted ? [2, 6] : [6, 4]
  const size = muted ? 1 : 1.5
  const slText = lineKind === 'confirmed'
    ? (isEn.value ? `SL -${riskRewardMetrics.value.riskPct.toFixed(1)}%` : `止损 -${riskRewardMetrics.value.riskPct.toFixed(1)}%`)
    : lineKind === 'sim'
      ? (isEn.value ? `SL sim` : '试算止损')
      : (isEn.value ? 'SL requested' : '待确认止损')
  const tpText = lineKind === 'confirmed'
    ? (isEn.value ? `TP +${riskRewardMetrics.value.rewardPct.toFixed(1)}%` : `止盈 +${riskRewardMetrics.value.rewardPct.toFixed(1)}%`)
    : lineKind === 'sim'
      ? (isEn.value ? `TP sim` : '试算止盈')
      : (isEn.value ? 'TP requested' : '待确认止盈')

  if (slPx > 0 && lineKind !== 'none') {
...
<template>
  ...
  >
    ...
    >
      ...
      <span class="t-faint num hidden text-xs lg:inline">1H ATR {{ currentAtr > 0 ? '$' + currentAtr.toFixed(currentAtr >= 100 ? 1 : 4) : '--' }}</span>
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-2 text-xs lg:grid-cols-4">
        ...
        <div class="rounded-lg border p-2.5" style="background-color: var(--surface-2); border-color: var(--line-1)">
          ...
          <div class="mt-1 flex items-baseline gap-1.5">
            <span class="num text-lg font-bold up">{{ riskRewardMetrics.estProfitUsd == null ? '--' : '+$' + riskRewardMetrics.estProfitUsd.toFixed(2) }}</span>
        ...
        <div class="rounded-lg border p-2.5" style="background-color: var(--surface-2); border-color: var(--line-1)">
          ...
          <div class="mt-1 flex items-baseline gap-1.5">
            <span class="num text-lg font-bold down">{{ riskRewardMetrics.estRiskUsd == null ? '--' : '-$' + riskRewardMetrics.estRiskUsd.toFixed(2) }}</span>
