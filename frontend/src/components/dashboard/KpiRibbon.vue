...
const ocoCoverage = computed(() => {
  ...
  if (!total) return { pct: null as number | null, missing: 0 };
  const ok = store.positions.filter((p) => isProtectionConfirmed(p)).length;
...
<template>
  <div class="card space-y-2.5 p-2.5 xl:p-3">
    ...
    <BaseStat
      ...
      :value="ocoCoverage.pct == null ? '--' : `${ocoCoverage.pct}%`"
      :delta="ocoCoverage.pct == null ? undefined : ocoCoverage.missing ? t('dash.matrix.kpi.missN', undefined, { n: ocoCoverage.missing }) : t('dash.matrix.kpi.allCovered')"
      :delta-tone="ocoCoverage.pct === 100 ? 'up' : ocoCoverage.pct == null ? 'muted' : 'warn'"
