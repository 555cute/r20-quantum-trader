...
/** 持仓 ⇄ 挂单 分段面板：行点击联动图表选币；保护状态 Fail-Closed */
import { computed, ref } from 'vue';
import { useDashboardStore } from '../../stores/dashboard';
import { useI18n } from '../../composables/useI18n';
import { fmtNum, fmtSigned, fmtPct, fmtPrice, arrow } from '../../utils/format';
import { isProtectionConfirmed, isPairedConditional } from '../../utils/protection';
...
function posPnl(p: PositionItem): number {
  return Number(p.upl ?? 0);
}
function posRoi(p: PositionItem): number {
  return Number(p.roi_pct ?? p.uplRatio ?? 0);
}
function orderDir(o: { posSide?: string; side?: string }): 'long' | 'short' {
  return String(o.posSide || (o.side === 'buy' ? 'long' : 'short')).toLowerCase() as 'long' | 'short';
}
function qtyOf(v: unknown): string {
  const n = Number(v);
  if (!Number.isFinite(n)) return '--';
  return `${fmtNum(n, Math.abs(n) >= 1 ? 2 : 4)} BASE`;
}
</script>
...
<template>
  <div class="card flex h-full flex-col overflow-hidden">
    ...
    <div class="flex items-center gap-2 border-b px-3 py-2.5" style="border-color: var(--line-1)">
      ...
      />
      <span v-if="store.usesPairedConditional" class="t-faint ms-auto hidden max-w-[280px] truncate text-2xs xl:block" :title="t('dash.matrix.positions.pairedHint')">
        {{ t('dash.matrix.positions.pairedHint') }}
      </span>
      <span v-else-if="tab === 'positions' && !positions.length" class="t-faint ms-auto hidden text-xs sm:block">
          ...
          >
            <td>
              <div class="flex items-center gap-2">
                <span class="num font-semibold" style="color: var(--ink-strong)">{{ symOf(p.instId) || p.name }}</span>
                <DirTag :dir="p.side" />
              </div>
              <p class="t-faint text-2xs leading-tight">{{ qtyOf(p.pos) }}<span v-if="p.stageDesc"> · {{ p.stageDesc }}</span></p>
            </td>
            ...
            <td class="text-center">
              <span v-if="isProtectionConfirmed(p)" class="badge badge-up" :title="isPairedConditional(p) ? t('dash.matrix.positions.pairedHint') : t('dash.matrix.positions.ocoOk')">
              ...
              <span v-else class="badge badge-warn" :title="t('dash.matrix.positions.ocoMissHint')">
                {{ p.protectionStatus ? t('dash.matrix.positions.ocoPending') : t('dash.matrix.positions.ocoMiss') }}
              </span>
            </td>
          </tr>
        </tbody>
      ...
      <table v-else class="table">
        ...
        <tbody>
          ...
          >
            <td>
              <div class="flex items-center gap-2">
                <span class="num font-semibold" style="color: var(--ink-strong)">{{ symOf(o.instId) || o.name }}</span>
                <DirTag :dir="orderDir(o)" />
              </div>
            </td>
            <td class="col-num">{{ fmtPrice(o.px) }}</td>
            <td class="col-num">{{ qtyOf(o.sz) }}</td>
