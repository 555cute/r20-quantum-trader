...
export const zhMatrix = {
  ...
  positions: {
    ...
    unclassifiedPill: '腿认不出',
    unclassifiedHint: '该所存在**读到了但认不出**的保护腿（无本方标签、类型名也不认识，'
      + '或行本身解析不了）。它们**不计入覆盖** ⇒ 若其实是保护腿，覆盖会被**低估**'
      + '（可能触发重复挂腿），须人工核对。',
    mismatchPill: '腿归属存疑',
    mismatchHint: '该所存在方向或量与任何持仓都对不上的保护腿：方向不符的**不计入覆盖**'
      + '（反向腿保护不了本仓）；量不符的**仍被计入覆盖**但归属存疑，可能是旧仓遗留'
      + '（价格触及仍会减仓），须人工核对。',
