/** 实盘矩阵：KPI 带 / 持仓 / 挂单 / K线工作站 / 因子矩阵 */
export const zhMatrix = {
  title: '实盘矩阵',
  desc: '账户、持仓、挂单与全标的因子动能同屏监控，AI 主脑每 15 分钟裁决一次',

  kpi: {
    equity: '账户总权益',
    todayPnl: '今日已实现',
    floatPnl: '持仓浮动盈亏',
    ls: '多空持仓比',
    margin: '保证金占用',
    oco: '已确认保护',
    equityTip: '钱包余额 + 全部持仓未实现盈亏',
    todayTip: '今日 00:00（UTC+8）以来平仓实现的净盈亏',
    floatTip: '在途持仓按标记价计算的未实现盈亏',
    marginTip: '已用保证金 / 账户总权益',
    ocoTip: '仅核验为完整覆盖的保护计为已挂。缺失、未知、部分成交或撤单中均不算武装。Binance 配对条件单不是原子 OCO。',
    allCovered: '全覆盖',
    missN: '{n} 笔缺失',
    day14: '近 14 日',
    vsYesterday: '较昨日',
  },

  positions: {
    tab: '活动持仓',
    title: '活动持仓',
    empty: '当前空仓，AI 正在等待更好的入场结构',
    col: {
      symbol: '标的',
      dir: '方向',
      qty: '数量 (BASE)',
      entry: '开仓均价',
      mark: '标记价',
      liq: '强平价',
      margin: '保证金',
      lev: '杠杆',
      pnl: '未实现盈亏',
      roi: '收益率',
      sl: '止损',
      tp: '止盈',
      oco: '保护',
    },
    ocoOk: '已确认',
    ocoMiss: '未确认',
    ocoPending: '待核验',
    ocoMissHint: '保护未经核验；请求中或待确认的止损不算已武装。',
    pairedHint: 'Binance 配对止盈止损是相互独立的条件单，不是原子 OCO。',
  },

  orders: {
    tab: '在途挂单',
    title: '在途限价挂单',
    empty: '无在途挂单，AI 将在下个周期按研判重新布单',
    col: {
      symbol: '标的',
      dir: '方向',
      type: '委托',
      price: '委托价',
      qty: '数量 (BASE)',
      sl: '预设止损',
      tp: '预设止盈',
      placed: '挂单时间',
      state: '状态',
    },
    decisionTime: '推理时间',
    cancel: '撤销',
    aiManaged: '挂单由 AI 动态管理：行情偏离或时机变化时会自动撤改',
    cancelTitle: '撤销挂单',
    cancelDesc: '{sym} {dir} 限价单 @ {price} 将被撤销，AI 会在下个周期重新评估入场时机。',
    canceled: '挂单已撤销',
    typeMaker: '限价',
    typeAlgo: '条件单',
  },

  chart: {
    title: 'K 线工作站',
    perp: '永续',
    holding: '持仓',
    overlays: '主图叠加',
    panes: '副图窗口',
    tf: '周期',
    indicators: '指标',
    indicatorsN: '指标 {n}',
    simulate: '试算',
    fullscreen: '全屏',
    exitFullscreen: '退出全屏',
    pickHint: '点击在图表中查看该标的',
    resetView: '重置视图',
    barCloseIn: '本根收盘',
    atr: '1H ATR',
    live: '实时',
    empty: '标的池暂无行情',
    posLines: '持仓线',
    ocoLines: '止盈止损线',
    vwap: 'VWAP',
    ma: '均线',
    boll: '布林带',
    vol: '成交量',
    macd: 'MACD',
    rsi: 'RSI',
    kdj: 'KDJ',
    superTrend: '超级趋势',
    indicatorHint: '点击切换显示，最多同时 4 组',
    sim: {
      simulate: '试算',
      enter: '进入 R:R 试算',
      exit: '退出试算',
      title: 'R:R 试算台',
      linked: '联动实盘持仓',
      spec: '假设测算',
      rrOk: '期望盈亏比',
      rrLow: '盈亏比不足',
      rrMin: '底线 2.0R',
      atr: '止损呼吸空间',
      atrOk: '防插针区间',
      atrDev: '偏离 1.8~2.2x',
      tp: '预期收益',
      sl: '最大风险',
      reset: '复位',
      copy: '复制风控参数',
    },
  },

  matrix: {
    title: '因子动能矩阵',
    desc: '1H 微积分动力学 × 聪明钱筹码 × AI 终审结论，点击行看完整证据链',
    empty: '等待首个因子快照',
    col: {
      symbol: '标的',
      price: '现价',
      chg: '24h 涨跌',
      vel: '速度 v',
      acc: '加速度 a',
      adx: 'ADX',
      cmf: 'CMF',
      smart: '大户多头',
      ls: '多空比',
      decision: 'AI 结论',
      conf: '置信',
    },
    velTip: '价格一阶导（1H）：上涨动能方向',
    accTip: '价格二阶导（1H）：动能加速或衰竭',
    adxTip: '趋势强度（ADX）。缺失显示 --。',
    cmfTip: '资金流方向：正值净流入',
    regime: {
      trendUp: '多头趋势',
      trendDown: '空头趋势',
      range: '震荡区间',
      chaos: '杂波',
    },
    smartTip: '所选适配器发布的账户多空比；缺失显示 --',
    detailTitle: '因子透视 · {sym}',
    market: '行情快照',
    calculus: '微积分动力学',
    macro: '宏观研判',
    decisionDetail: '决策明细',
    entry: '计划入场',
    sl: '计划止损',
    tp: '计划止盈',
    rr: '盈亏比',
    reason: '裁决理由',
    noDecision: '本周期该标的无动作',
    updated: '快照 {t}',
  },
};
