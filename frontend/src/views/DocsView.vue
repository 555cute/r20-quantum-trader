      ...
      <main class="min-w-0 flex-1 space-y-14 pb-24">
        ...
        <section id="overview" class="space-y-4 pt-2">
          ...
          <p class="text-xs sm:text-sm leading-relaxed font-sans" style="color: var(--ink-2);">
            <strong>R20量子交易系统 (R20 Quantum Trading System)</strong> 是一套专为高波动加密货币（Crypto）打造的<strong>机构级全自动波段量化决策与执行系统</strong>。系统对接<strong>当前所选交易所</strong>（首发 Binance USD-M 永续，亦可配置 OKX），运行在严格的北京时间（UTC+8）自然日财务基准之上，聚焦 1H~4H 大级别顺势波段，以<strong>“胜率第一、宁缺毋滥、三位一体 Fail-Closed 物理硬防线”</strong>为最高风控宗旨。Binance 配对止盈止损是相互独立的条件单，<strong>不是原子 OCO</strong>；仅交易所核验的完整保护才计为已挂。在 v7.5.0 全面集成本地化 TradingView 官方轻量引擎与视觉 LLM 友好型量价形态工作站；v7.6.0 里程碑将全部执行层硬风控阈值从源码剥离，升级为后台可视化「风控管理中心」与三套优质预设套件。
          </p>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
            <div class="p-4 rounded-xl border space-y-2 shadow-xs" style="background-color: var(--surface-2); border-color: var(--line-1);">
              ...
              <p class="text-xs leading-relaxed" style="color: var(--ink-2);">
                绝不将风控寄托于 LLM 提示词本身。在交易执行底层设立不可覆盖的 Python 物理拦截插件管线。保护必须 Fail-Closed：缺失、未知、部分成交或撤单中均不算武装。Binance 条件单腿相互独立，不能当成原子 OCO。
              </p>
            </div>
            ...
            <table class="w-full text-left text-xs whitespace-nowrap">
              ...
              <tbody class="divide-y" style="border-color: var(--line-1);">
                ...
                <tr class="hover:bg-[var(--surface-3)] transition-colors">
                  ...
                  <td class="p-3" style="color: var(--ink-2);">注入在途持仓方向、均价、标记价、未结浮盈 UPL 及已核验保护（数量单位为 BASE）</td>
                </tr>
                <tr class="hover:bg-[var(--surface-3)] transition-colors">
                  <td class="p-3 font-bold" style="color: var(--accent);">&#123;&#123;pending_orders&#125;&#125;</td>
                  <td class="p-3" style="color: var(--ink-1);">挂单池</td>
                  <td class="p-3" style="color: var(--ink-2);">注入在途 Maker 限价挂单价格、BASE 数量及被动成交状态</td>
                </tr>
              </tbody>
            </table>
          </div>
...
./scripts/start_standalone.sh</pre>
        ...
        <section id="faq" class="space-y-4 pt-6 border-t" style="border-color: var(--line-1);">
          ...
          <div class="space-y-3">
            ...
            <div class="p-4 rounded-xl border space-y-2 shadow-xs" style="background-color: var(--surface-2); border-color: var(--line-1);">
              <h3 class="text-sm font-bold" style="color: var(--ink-1);">Q2: 我的交易所 API Key 和大模型密钥会泄露吗？</h3>
              <p class="text-xs leading-relaxed" style="color: var(--ink-2);">
