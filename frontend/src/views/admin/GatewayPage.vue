...
import { Zap, RefreshCw, RotateCcw, Server, Clock, AlertTriangle, Play } from 'lucide-vue-next'
...
async function runTrader() {
  ...
  try {
    ...
    toast.ok(`${ex} ${mode} 交易主脑已触发`)
    await load()
  } catch (e: any) {
    toast.err(`运行失败：${e.message}`)
  }
}
...
<template>
  <div class="space-y-4">
    <PageHeader :title="t('nav.admin.gateway')" description="调度任务、事件投递队列与死信重放；Gateway 仅记录无内容遥测。">
      <template #actions>
        <span class="chip">日常运行 · <b class="num">4/4</b></span>
      </template>
    </PageHeader>
            ...
            <tbody>
              <tr v-for="j in gw.scheduler.jobs" :key="j.name" class="border-b last:border-b-0 hover:bg-[var(--surface-3)] transition-colors" style="border-color: var(--line-1);">
                ...
                <td class="py-2.5 px-4 text-right font-bold" :class="j.overdue ? 'text-rose-400' : 'text-emerald-400'">
                  <button v-if="j.name === 'trader'" @click="runTrader" class="mr-2 px-2 py-1 rounded-md border text-[10px] cursor-pointer inline-flex items-center gap-1" style="background-color: var(--warn-bg); border-color: var(--warn-line); color: var(--warn);">
                    <Play class="w-3 h-3" /><span>运行交易主脑</span>
                  </button>
                  {{ j.overdue ? '逾期' : '正常' }}
                </td>
              </tr>
