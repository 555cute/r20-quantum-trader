...
async function rediagnose() {
  bannerMsg.value = { text: isBinanceSaved.value ? '正在读取当前交易所账户状态…' : '正在检查 OKX CLI、OAuth 与私有读取…', type: 'warn' }
  try {
...
function requireSavedVenue(): boolean {
  ...
  bannerMsg.value = { text: `交易所草稿为 ${draftExchange.value.toUpperCase()}，标的/持仓仍按已保存的 ${savedExchange.value.toUpperCase()}。请先保存环境与凭证。`, type: 'warn' }
  return false
}
    ...
    <template v-else-if="config">
      ...
      <div class="rounded-xl border overflow-hidden shadow-xs" style="background-color: var(--bg-card); border-color: var(--border-subtle);">
        <div class="px-4 py-3 border-b flex items-center justify-between" style="border-color: var(--border-subtle); background-color: var(--bg-card-subtle);">
          ...
          <button @click="loadPositions" :disabled="exchangeDraftDirty" class="flex items-center space-x-1.5 px-2.5 py-1 rounded-lg border text-xs font-mono cursor-pointer transition-all shadow-xs disabled:opacity-40" style="background-color: var(--bg-card); border-color: var(--border-medium); color: var(--text-main);">
