...
function authOf(item: { authority?: string; source?: string }): { cls: string; label: string } {
  const a = String(item.authority || '').toLowerCase();
  if (a === 'official') return { cls: 'badge-up', label: t('dash.news.feed.authority.official') };
  if (a === 'media') return { cls: 'badge-warn', label: t('dash.news.feed.authority.media') };
  return { cls: '', label: t('dash.news.feed.authority.unverified') };
}
function sourceOf(item: { source_name?: string; source?: string }): string {
  return String(item.source_name || item.source || '').trim();
}
</script>
  ...
  <div class="space-y-3">
    ...
    <div class="grid grid-cols-1 gap-3 xl:grid-cols-12">
      ...
      <div class="card overflow-hidden xl:col-span-8">
        ...
        <p class="border-b px-3.5 py-2 text-xs leading-relaxed" style="border-color: var(--line-1); color: var(--ink-2)">
          {{ t('dash.news.feed.mediaRule') }}
        </p>
        ...
        <div v-else class="max-h-[640px] divide-y overflow-y-auto" style="--tw-divide-y-reverse:0">
          ...
          >
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="badge" :class="authOf(item).cls">{{ authOf(item).label }}</span>
              <span v-if="sourceOf(item)" class="badge badge-mono">{{ sourceOf(item) }}</span>
