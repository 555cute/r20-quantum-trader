...
def admin_okx_runtime(x_r20_session: str | None = Header(default=None, alias="X-R20-Session"), refresh: int = 0) -> dict[str, Any]:
    """纯 API Key 连接诊断（V5 直签是唯一连接方式；CLI/OAuth 已彻底移除）。"""
    refresh_settings()
    require_admin_header(x_r20_session=x_r20_session)
    now = time.time()
    ...
    mode_label = env.mode.upper()
    payload: dict[str, Any] = {
        "environment": env.mode,
        "mode_configured": env.configured,
        "fingerprint": env.fingerprint if env.configured else "not-configured",
        "base_url_ok": env.base_url == "https://www.okx.com",
        "not_ready_reason": None if env.configured else f"未配置 {mode_label} API Key：系统 NOT READY 禁止交易，请在「账户接入」填写 LIVE/DEMO 三件套",
    }
...
def admin_okx_account_snapshot(x_r20_admin_token: str | None = Header(default=None)) -> dict[str, Any]:
    require_admin_header(x_r20_admin_token)
    # US-009 fail-closed：V5 直签是唯一连接方式，未配置 API Key 时 503 人话指引（CLI/OAuth 回退已删）。
    if not (settings.okx_demo_configured if settings.okx_environment == "demo" else settings.okx_live_configured):
        raise HTTPException(status_code=503, detail=f"OKX {settings.okx_environment.upper()} API Key 未配置：系统 NOT READY，请在后台「账户接入」填写三件套")
