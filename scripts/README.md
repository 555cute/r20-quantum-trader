...
| `gateway_liveness.py` | 121 | 由 `r20_watchdog.sh gateway` 每 30s 调用 | 网关存活判据（**退出码即语义**：0 健康 / 1 进程不存在 / 2 心跳停更 / 3 心跳缺失）。判据是 worker 每轮循环写的存活心跳，而不是周期产物够不够新 —— 用户**可以合法关停交易**，那时周期产物本就不更新，用它当判据会造成误杀循环 |
