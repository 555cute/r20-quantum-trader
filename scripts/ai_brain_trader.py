...
def construct_full_market_prompt(packages: List[Dict[str, Any]], pos_summary: str = "[MISSING_CONTEXT:account_positions]", active_positions_detail: List[Dict[str, Any]] = None, pending_orders_detail: List[Dict[str, Any]] = None, current_time_str: str = "", usdt_available: float = None, runtime_context_out: Dict[str, Any] = None, policy_snapshot: Dict[str, Any] = None) -> str:
    ...
    for p in packages:
        ...
        if sm.get("available", False):
            sm_line = (f"加权做多占比={sm.get('weighted_long_pct', 50)}% | 24H净流入={sm.get('net_flow_usdt', '--')} | "
                       f"多头均价={sm.get('avg_long_entry', '--')} | 空头均价={sm.get('avg_short_entry', '--')} | {sm.get('top_win_rate', '')}")
        else:
            sm_line = f"⚠️数据源缺失（{sm.get('reason', 'OKX CLI 已移除，无公开 V5 等价源')}），本项不参与决策"
...
- 👑 顶级聪明钱 (SmartMoney Top100): {sm_line}
...
def execute_batch_ai_brain_cycle(
    ...
    # OKX Smart Money Signals — OKX CLI 已彻底移除（服务器重启丢 OAuth 根因），
    # 官方核查无公开 CEX V5 等价端点：信号显式缺失化（available=False），
    # 决策提示词按「数据源缺失」渲染，禁止以 50%/0U 冒充实测；
    # 其余证据面（盘口/衍生品 REST/跨所比对）不受影响。
    for p in packages:
        p["smart_money"] = {
            "available": False,
            "reason": "OKX CLI 已移除；smartmoney 无公开 V5 等价接口，待接新数据源",
