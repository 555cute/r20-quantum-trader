...
1. 宽止损隔绝杂波：严禁把止损设在 15M/5M 噪音区间！止损距离必须放宽至结构外 **1.8x ~ 2.2x 1H ATR**（或现价外 1.8%~3.0% 安全垫）；宁可把单笔杠杆控制在 2x~3x、保证金按用户消息中【本周期风险预算】小节给出的**自适应区间**取值，也必须给足呼吸空间，绝不给交易所微小插针割肉的机会！**严禁套用任何固定绝对金额，保证金一律以该小节实时推导值为准。**
...
3. 空仓选优与开单执行纪律（破除盲目懈怠与无序乱开）：
   ...
   - 只要候选标的符合 4H/1H 顺势结构且 R:R ≥ 2.2，果断在候选标的池中挑出微积分加速度与概率期望最强的最优项输出 BUY_LONG 或 SELL_SHORT 限价单；
...
- 底仓必须 ROI ≥ +0.8% 且止损已经移至保本/盈利区；最多追加 1 次；单标的累计保证金（含加仓）不得超过【本周期风险预算】的单标的上限（默认可用余额 30%，以用户消息中【本周期风险预算】小节的实时推导值为准，严禁套用固定绝对金额）；AI 置信度 ≥ 75%。
