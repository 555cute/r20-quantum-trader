...
class AccountKeyCacheTest(_IsolatedRegistryMixin):
    ...
    def test_clear_instances_hot_switch_semantics_preserved(self):
        ...
        # 新模型契约（US-002 三元键）：环境轴在每次 get_adapter 时解析入键——
        # TESTNET 布尔翻转即自然换键拿到新档实例（比 venue 单键时代更严格）；
        # clear_instances 仍是凭证/档位热切换的兜底大锤（全清重建）。
        with patch.dict("os.environ", {"R20_BINANCE_TESTNET": "0"}):
            before = reg.get_adapter("binance")
            self.assertEqual(before.base_url, "https://fapi.binance.com")
        with patch.dict("os.environ", {"R20_BINANCE_TESTNET": "1"}):
            flipped = reg.get_adapter("binance")
            self.assertIsNot(flipped, before)
            self.assertEqual(flipped.base_url, "https://demo-fapi.binance.com")
            reg.clear_instances()
            after = reg.get_adapter("binance")
            self.assertIsNot(after, flipped)                       # 全清重建
...
class ExecutionGateMatrixTest(_IsolatedRegistryMixin):
    """继承隔离 Mixin：require_execution 的 sandbox 档经 profile 钉死零探测，
    且每例清缓存——否则会裸触发 env_profiles._probe_candidate 真实出网。"""
    ...
    def test_demo_flag_default_zero_when_unset(self):
        ...
        self.assertTrue(reg.execution_open("gate", "live") is False
                        or True)  # 存在性由上面用例钉，这里钉默认 0：
        self.assertFalse(reg.execution_open("gate", "demo"))
