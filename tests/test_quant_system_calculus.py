...
class AiFactorTraderPositionProtectionTest(unittest.TestCase):
    ...
    def test_stale_order_query_failure_aborts_cleanup(self):
        with patch.object(ai_factor_trader,"pending_orders",side_effect=RuntimeError("timeout")):
            ok,detail=ai_factor_trader.clean_stale_open_orders()
        self.assertFalse(ok); self.assertIn("timeout",detail)

    def test_stale_order_cancel_fail_closed(self):
        order={"instId":"SOL-USDT-SWAP","ordId":"11","state":"live","cTime":"1"}
        with patch.object(ai_factor_trader,"pending_orders",return_value=[order]), \
             patch.object(ai_factor_trader,"cancel_order",side_effect=RuntimeError("rejected")) as cancel, \
             patch.object(ai_factor_trader.time,"time",return_value=1000):
            ok,detail=ai_factor_trader.clean_stale_open_orders()
        self.assertFalse(ok); self.assertIn("rejected",detail)
        cancel.assert_called_once_with("SOL-USDT-SWAP","11")
